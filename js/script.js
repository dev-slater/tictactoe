...
//null = in progress, T=tie, 1 or -1 who won 
...
function initGame(){
    //Null is empty, 1 === X, -1 === 0
    gameState = [null,null,null,null,null,null,null,null,null];
    currentTurn = 1;
    winner = null;
    renderBoard();
    $winnerMessage.text("");
    $playerMessage.text(`it is currently player ${currentTurn}`);
};
...
function renderBoard(){
    gameState.forEach((squareMarker, gameStateIndex) => {
        ...
        //eq is same thing as brackets in vanilla js
        console.log("===========game");
...
// event listeners to squares with jquery 
...
// if any of the win combos add up to either 3 or -3, then the player whos turn it is has won
//iterate over the win conbos array, add to 3 or -3

function checkWinner() {
    winCombos.forEach((winCond) => {
        let points = 0;
        winCond.forEach((gameStateCheck) =>{
            points += gameState[gameStateCheck];
            console.log(points);
